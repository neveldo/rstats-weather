...
Dans le code R ci-dessous, les données ont déjà été préalablement nettoyées, structurées et stockées au sein d'un [triplestore RDF](https://fr.wikipedia.org/wiki/Triplestore) que nous pouvons donc directement interroger en **[SPARL](https://fr.wikipedia.org/wiki/SPARQL)** à l'aide du package `{SPARQL}`.
